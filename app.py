...
def iniciar_servidor():
    return {
        ...
        "conectados": set() # Lista de casas que han ingresado
    }
...
if rol == "Administrador":
    ...
    if clave == "Alameda2026*":
        ...
        st.subheader(f"👥 Casas Conectadas: {len(servidor['conectados'])}")
        with st.expander("Ver lista de casas en línea"):
        ...
        with col_admin1:
            if st.button("🚀 LANZAR PREGUNTA (Votación)", type="primary", use_container_width=True):
        ...
        if not servidor['votos'].empty:
            ...
            st.download_button("📥 Descargar Excel Final (CSV)", data=csv, file_name="votos_finales.csv")
            st.write("Vista previa de votos:")
            st.dataframe(servidor['votos'].tail(5))
    ...
    if 'mi_casa' not in st.session_state:
        ...
        if st.button("Entrar"):
            if c_in:
                ...
                servidor['conectados'].add(c_in) # Se añade a la lista de conectados
                st.rerun()
    else:
        casa = st.session_state.mi_casa
        st.sidebar.info(f"Casa: {casa}")
        
        # LÓGICA DE PANTALLA SEGÚN FASE DEL ADMIN
        fase = servidor['fase']
        p_id = servidor['p_idx']
        
        if fase == "espera":
            st.warning("⏳ Esperando a que la Administración inicie la votación...")
            ...
            # Verificar si ya votó en ESTA pregunta
            df = servidor['votos']
            ...
            if fase == "resultados":
                ...
                if not res.empty:
                    ...
                    ax.pie(res, labels=res.index, autopct='%1.1f%%', startangle=90, colors=['#2ecc71', '#e74c3c'])
                    ax.axis('equal')
                    st.pyplot(fig)
                else:
                ...
                st.warning("✅ Voto registrado con éxito. Espere la siguiente pregunta.")
